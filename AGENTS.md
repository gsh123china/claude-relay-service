...
> 兼容性说明：OpenAI Responses (Codex) 账户触发的账户异常/恢复等通知，其 Webhook payload 中的 `platform` 字段统一为 `openai`，以兼容现有接收端的错误码映射与告警规则。
...
- `CLAUDE_CONSOLE_ERROR_THRESHOLD`: Console 故障转移-失败次数阈值（默认10）
- `CLAUDE_CONSOLE_ERROR_WINDOW_MINUTES`: Console 故障转移-错误计数窗口（分钟，默认5）
- `CLAUDE_CONSOLE_TEMP_DISABLE_MINUTES`: Console 故障转移-临时禁用时长（分钟，默认10）
...
- Redis 键：
  ...
  - 临时禁用：`claude_console_account:temp_error:{id}`
  - 分布式锁：`claude_console_account:temp_error:lock:{id}`
- 说明：旧版“400错误消息模式匹配”机制已移除；`CLAUDE_CONSOLE_BLOCKED_HANDLING_MINUTES` 已废弃。
